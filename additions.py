...
def get_list():
	sql = "SELECT A.borough, A.genre, A.coordinates, U.username, A.sent_at, A.id FROM additions A, users U WHERE A.creator_id=U.id AND visible=1 ORDER BY A.sent_at DESC" 
	result = db.session.execute(sql)
	return result.fetchall()

def get_addition_info(addition_id):
    	sql = "SELECT a.borough, a,genre, u.username FROM additions a, users u WHERE a.id=:addition_id AND a.creator_id=u.id;"
...
def send(borough, genre, coordinates):
	...
	if creator_id == 0:
		return False
	sql = "INSERT INTO additions (borough, genre, coordinates, creator_id, sent_at, visible) VALUES (:borough, :genre, :coordinates, :creator_id, NOW(), 1)"
...
def get_my_additions(user_id):
    sql = "SELECT id, borough, genre, coordinates FROM additions WHERE creator_id=:user_id AND visible=1 ORDER BY id"
...
def add_review(addition_id, stars, comment, user_id,):
    sql = "INSERT INTO reviews (addition_id, stars, comment, user_id) VALUES (:addition_id, :stars, :comment, :user_id)"
